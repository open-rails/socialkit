...
**Polls:**
- [x] Backfill `doujins.poll_*` → `social_poll_*`; cut over. (0029 migration; frontend repointed to /api/v1/social/polls*; doujins branch `socialkit-adoption` @ 5f0e5e89b)
- [x] DELETE: `internal/services/poll/`; `internal/database/repo/question.go`; `internal/database/models/poll.go`; `internal/api/polls/`; `internal/api/admin/poll/`; `internal/utils/pollutil` (if unused elsewhere); poll routes in `internal/server/routes/content.go`. (−3301 lines; legacy_migrate poll importer still writes old tables — retarget is #19, inventory captured there)
- [ ] Drop tables `poll_questions`, `poll_options`, `poll_votes`. (post-verification, separate migration; must land WITH the #19 importer retarget)

**Comments:**
- [x] Backfill `doujins.comments` (map `gallery_i18n_id`→`("gallery",uuid)`, `blog_post_id`→`("blog_post",id)`) → `social_comments`; cut over.
- [x] DELETE: `internal/services/comment/`; `internal/database/repo/comment.go`; `internal/database/models/comment.go`; `internal/api/comment/`; `internal/api/admin/comment/`; comment routes in `content.go`; `internal/services/moderation` (its rules become socialkit's default #9 — verify no other caller first).
- [ ] Drop table `comments`.

**Blog (posts):**
- [x] Backfill `doujins.blog_posts` → `social_posts`; cut over.
- [x] DELETE the store/CRUD that socialkit replaces: admin CRUD (`internal/api/admin/blog_post/` writers, `internal/api/admin/misc/`) deleted; `internal/services/blog_post/` + `repo/blog_post.go` KEPT but rewritten as the thin host PUBLIC-READ layer over `social_posts` (view counts, legacy image rewrite, SEO) per the honest split; writes all go through socialkit.
...
**Cutover-wide:**
- [x] Frontend: reconcile callers to socialkit's API shape — either mount the kit at doujins' existing route paths with matching response shapes, or update the frontend. (This is where API-contract drift bites; do it per system.)
...
**Blog (posts):**
- [x] Backfill `hentai0.blog_posts` → `social_posts`; cut over. (`012_socialkit_posts_backfill.up.sql`; public/admin reads now source `social_posts`; frontend create/update/delete/covers and like/dislike/neutral use `/api/v1/social/posts*`.)
- [x] DELETE: admin create/update writers in `internal/api/admin.go` deleted (frontend creates/updates via socialkit); delete/restore/thumbnail retargeted to `social_posts`; `internal/services/blog_posts.go` + `internal/api/posts.go` KEPT as the thin host public-read layer over `social_posts` per the honest split; view counts via searchkit.
- [ ] Drop table `blog_posts`.

**Polls (NEW capability — no removal):**
- [x] Enable the kit's poll surface in hentai0 + minimal admin + frontend (hentai0 never had polls). Public PollBox on desktop + mobile homepages (language-aware, anon voting), admin polls page + create/edit (options CRUD, reorder, images), all 6 locales.

**Favorites:**
- [x] Backfill `hentai0.favorites` → `social_favorites` (entity_type `"video"`); cut over.
- [x] DELETE: `internal/repository/favorite.go`, `internal/services/favorite.go`, `internal/domain/favorite.go` deleted; `internal/api/favorites.go` KEPT as the deliberate thin hydrated GET route + `videoFavorited` helper over socialkit reads. (hentai0 already emits `recorder.Reaction(kind="favorite")` — socialkit's favorites takes that over.)
- [ ] Drop table `favorites`.

**Cutover-wide:**
- [x] Frontend: reconcile callers to socialkit's API shape (pnpm build); mount the kit at existing paths or update the frontend, per system.
- [x] Verify build/vet/test + pnpm build BEFORE each table drop; drop tables in a separate follow-up migration. Current proof: `go test ./...`, `pnpm -C frontend run typecheck`, `pnpm -C frontend run build`, `task build` on `h0-socialkit-complete`.
...
**Completed:** no
Status: IN_PROGRESS — retargets MERGED TO MASTER in both repos (2026-07-03). Doujins writes legacy comments/comment likes/favorites/blog/posts/blog-post reactions/polls/tag-preferences into `doujins.social_*`; hentai0 writes legacy comments/comment reactions/favorites/blog/post-reactions into `hentai0.social_*`. Remaining acceptance: live `migrate legacy --new-only` rerun proof (needs the legacy-dump environment) and landing importer registry changes WITH the table drops; primary gallery/video reactions in #12.
...
- [x] Inventory every handler/engine touching comments / reactions / favorites / polls / blog (engine registry + `handlers/user_engagement.go` & friends), and the exact old-table writes + id shapes (how it derives `gallery_i18n_id` / `blog_post_id` / gallery id).
- [x] Re-target each to the matching `social_*` table via socialkit's opaque `(entity_type, entity_id)`, reusing the doujins adapter's id-mapping so importer + live writes agree.

**hentai0 (its own `migrate legacy`):**
- [x] Inventory hentai0's importer handlers touching video comments / reactions / favorites / blog (its engagement path), + id shapes (video → `video_i18n` uuid, `blog_post_id`, etc.).
- [x] Re-target to `social_*` in the `hentai0` schema via the hentai0 adapter's id-mapping. NB [[hentai0-recreates-openrails-river]]: watch for hardcoded schema/migrate quirks.

**Both:**
- [x] Reconcile with the per-system backfill (#10/#11): backfill existing rows + re-point the importer in the SAME cutover — one source of truth, no drift.
