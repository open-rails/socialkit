...
func TestReactions_TransitionsAndCounts(t *testing.T) {
	...
	must(rt.reactions.react(ctx, actor, "widget", "1", 1)) // like
	assertCounts(t, rt, actor, "widget", "1", 1, 0, 1)
	must(rt.reactions.react(ctx, actor, "widget", "1", -1)) // switch to dislike
	assertCounts(t, rt, actor, "widget", "1", 0, 1, -1)
	must(rt.reactions.react(ctx, actor, "widget", "1", 0)) // neutral (not delete)
	assertCounts(t, rt, actor, "widget", "1", 0, 0, 0)

	// second distinct user likes -> independent row
	must(rt.reactions.react(ctx, Actor{ID: "u2", Kind: "user"}, "widget", "1", 1))
...
func TestReactions_ConcurrentDoubleLikeIsExact(t *testing.T) {
	...
	for i := 0; i < 20; i++ {
		...
		go func() {
			defer wg.Done()
			errs <- rt.reactions.react(context.Background(), actor, "widget", "42", 1)
		}()
	}
	wg.Wait()
	close(errs)
...
func TestReactions_GatingRejectsInaccessibleAndMissing(t *testing.T) {
	...
	if err := rt.reactions.react(ctx, actor, "widget", "locked", 1); !errors.Is(err, ErrForbidden) {
		t.Fatalf("react on premium-locked: want ErrForbidden, got %v", err)
	}
	if err := rt.reactions.react(ctx, actor, "widget", "hidden", 1); !errors.Is(err, ErrNotVisible) {
		t.Fatalf("react on hidden: want ErrNotVisible, got %v", err)
	}
	if err := rt.reactions.react(ctx, actor, "widget", "ghost", 1); !errors.Is(err, ErrNotFound) {
		t.Fatalf("react on missing: want ErrNotFound, got %v", err)
	}
	if err := rt.reactions.react(ctx, actor, "unregistered", "1", 1); !errors.Is(err, ErrNotFound) {
...
func TestReactions_AnonymousDedupByIP(t *testing.T) {
	...
	if err := rt.reactions.react(ctx, anon, "widget", "1", 1); err != nil {
	...
	if err := rt.reactions.react(ctx, Actor{Anonymous: true}, "widget", "1", 1); err == nil {
...
func TestReactions_RecorderSignalEmitted(t *testing.T) {
	...
	if err := rt.reactions.react(context.Background(), Actor{ID: "u1"}, "widget", "1", 1); err != nil {
		t.Fatalf("react: %v", err)
	}
